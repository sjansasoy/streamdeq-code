...
class TopDownMpiiVidDataset(Kpt2dSviewRgbImgTopDownDataset):
    ...
    def __init__(self,
        ...
        self.num_images = len(self.image_set) * (self.num_prev_frames + 1)
    ...
    def _get_db(self):
        ...
        gt_db = []
        bbox_id = 0
        mat_annots = loadmat(osp.join(self.img_prefix, '../mpii_human_pose_v1_sequences_keyframes.mat'))
        for a in anno:
            image_name = a['image']
            ...
            image_file = []
            frame_folder = osp.join(self.img_prefix, image_name.split('.')[0])
            for image in mat_annots['annolist_keyframes'][0]:
                vid_name, key_frame = image[0][0][0][0][0].split('/')
                if vid_name == image_name.split('.')[0]:
                    if os.path.exists(frame_folder):
                        all_frames = sorted(os.listdir(frame_folder))
                        if '.ipynb_checkpoints' in all_frames:
                            all_frames = all_frames.remove('.ipynb_checkpoints')
                        idx_keyframe = all_frames.index(key_frame)
                        for frame_idx in range(self.num_prev_frames, -1, -1):  # key frame + ref frames
                            image_file.append(osp.join(frame_folder, all_frames[idx_keyframe - frame_idx]))
                            
            gt_db.append({
                'image_file': tuple(image_file),
                'bbox_id': bbox_id,
                'center': center,
                'scale': scale,
                'rotation': 0,
