...
def gd(f, x0, threshold=50, eps=1e-3, stop_mode='rel', **kwargs):
    """ Gradient descent for fixed point iteration. """
    criterion = nn.CrossEntropyLoss()
    inputs = x0.clone().detach().requires_grad_(True)
    lr = 0.1
    optimizer = optim.SGD([inputs], lr=0.01)
    labels = torch.zeros_like(inputs)
    for k in range(500):
        with torch.enable_grad():
            Y = f(inputs.requires_grad_(True)) - inputs
...
def analyze_broyden(res_info, err=None, judge=True, name='forward', training=True, save_err=True):
    ...
    if nstep == 0 and (diff != diff or diff > eps):
        msg = f"WARNING: Bad Broyden's method {name}. Diff: {diff}. STOP."
        print(msg)
