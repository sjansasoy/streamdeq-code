...
class PreprocessH36m:
    ...
    def _load_annotations(self, subject, action, subaction, camera):
        ...
        subj_ann_dir = join(self.extracted_dir, 'annotations', subject)
        subj_img_dir = join(self.extracted_dir, 'images', subject)
        basename = self._get_base_filename(subject, action, subaction, camera)

        # load 2D keypoints
        with cdflib.CDF(
                join(subj_ann_dir, 'MyPoseFeatures', 'D2_Positions',
                     basename + '.cdf')) as cdf:
        ...
        # load 3D keypoints
        with cdflib.CDF(
                join(subj_ann_dir, 'MyPoseFeatures', 'D3_Positions_mono',
                     basename + '.cdf')) as cdf:
        ...
        imgnames = []
        all_videos = sorted(os.listdir(subj_img_dir))
        if ' ' in basename:
            name, action = basename.split(' ')
        else:
            name, action = basename.split('.')
        for idx, video in enumerate(all_videos):
            if name in video and action in video:
                my_images = sorted(os.listdir(join(subj_img_dir, video)))
        
        if '.ipynb_checkpoints' in my_images:
            my_images.remove('.ipynb_checkpoints')

        if len(my_images) > num_frames:  # inconsistency due to different number of frames per each camera
            my_images.pop(-1)

        for i in range(len(my_images)):  # sampling and adding necessary images to the list
            if i % self.sample_rate == 0:
                imgnames.append(my_images[i])
            if len(imgnames) == len(centers):
                    break

        imgnames = np.array(imgnames)

        print(f'Annotations for sequence "{subject} {basename}" are loaded. '
              f'{len(imgnames)}/{num_frames} samples in total.')
