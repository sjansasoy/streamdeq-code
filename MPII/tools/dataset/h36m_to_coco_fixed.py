...
def _get_img_info(img_idx, img_name, img_root):
    try:
        ...
        folder_name = img_name[:-11]
        im = Image.open(osp.join(img_root, subject, folder_name, img_name))
        w, h = im.size
    except:  # noqa: E722
        return None
...
def main():
    ...
    parser.add_argument(
        '--ann-file', type=str, default='data/h36m/annotation_body3d/fps10/h36m_train.npz')
    parser.add_argument(
        '--camera-param-file', type=str, default='data/h36m/annotation_body3d/cameras.pkl')
    parser.add_argument('--img-root', type=str, default='data/h36m/images')
    parser.add_argument(
        '--out-file', type=str, default='data/h36m/annotation_body2d/h36m_coco_train_one_img.json')
    parser.add_argument('--full-img-name', action='store_true')

    args = parser.parse_args()

    h36m_data = np.load(args.ann_file)
    d = dict(zip(("{}".format(k) for k in h36m_data), (h36m_data[k] for k in h36m_data)))
    ...
    h36m_imgs = mmcv.track_parallel_progress(_get_img_info, tasks[:2], nproc=12)
